  ...
  return (
    <RouletteContainer className="roulette-container">
     ...
     <RoulleteBtn
        ...
        className="start-btn ant-btn ant-btn-default ant-btn-round ant-btn-lg"
      >
        {start_text}
      </RoulleteBtn>
...
const RouletteBody = ({
  ...
  // prize_num = generate_random_number(0, prize_arr.length - 1)

  const prizes = [
    { number: 0, weight: 0.6 },
    { number: 1, weight: 0.2 },
    { number: 2, weight: 0.15 },
    { number: 3, weight: 0.10 },
    { number: 4, weight: 0.0125 },
    { number: 5, weight: 0.0125 },
  ];
  ...
  const prize_num = selectedPrize.number;
  ...
  const reset_roulette = () => {
    TweenLite.to([UBoardRef.current, OBoardRef.current], 0.8, {
      rotation: 0,
      ease: Power2.easeIn,
      onComplete: () => {
